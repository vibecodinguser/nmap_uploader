...
export const shouldRemountGoToToolbar = ({
  goToCount,
  splitCount,
  isGoToMountedCorrectly,
  isSplitSiblingCorrect,
}: GoToToolbarMountState): boolean =>
  goToCount > 1 ||
  splitCount > 1 ||
  (goToCount === 1 && !isGoToMountedCorrectly) ||
  (goToCount === 1 && splitCount === 1 && !isSplitSiblingCorrect)
...
export const repairAllGoToToolbarIcons = (iconSvgByButtonId: Record<string, string>): number => {
  ...
  for (const [buttonId, iconSvg] of Object.entries(iconSvgByButtonId)) {
    for (const button of queryAllByDomId(buttonId)) {
      ...
      if (repairGoToIconHost(iconHost, iconSvg)) repaired += 1
    }
  }
